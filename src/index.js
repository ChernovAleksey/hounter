document.querySelector(".navigation__main--button-prop").addEventListener("click", (event) => { 
  if (
    document
...
const inputAlarmer = (alarmtext) => {
  ...
  document.querySelector(".inputalarmer__close").addEventListener("click", (event) => {
...
document.querySelector(".header__hero--location-input").addEventListener("focus", (event) => {

  document.querySelector(".header__hero--location-input").value = '';

});

document.querySelector(".header__hero--location-search").addEventListener("click", (event) => {
  if(document.querySelector(".header__hero--location-input").value !=='Search for the location you want!')
  {inputAlarmer(`The chosen location is: ${document.querySelector(".header__hero--location-input").value}`);}
  document.querySelector(".header__hero--location-input").value = 'Search for the location you want!'
});

document.querySelector(".header__hero--location-input").addEventListener("blur", (event) => {
...
const renewRecomDataArray = () => {
  ...
  recomDataArray.forEach((item) => { recomendationArrayToDisplay.push(item); });
}
const recomendationDisplay = (arr) => {
  ...
  document.querySelector(".recomendation__itemLine").insertAdjacentHTML("afterBegin", recomendationDisplayString);
};


document.querySelector(".recomendation__chooseBattons").addEventListener("click", (event) => {
  if (
...
document.querySelector(".recomendation__navigationBattons").addEventListener("click", (event) => {
  if (event.target.closest("div").dataset.tabname === "left") {
    recomendationArrayToDisplay.unshift(recomendationArrayToDisplay[recomendationArrayToDisplay.length-1]);
    recomendationArrayToDisplay.pop(recomendationArrayToDisplay[recomendationArrayToDisplay.length-1]);
  } else {
...
const readyToSellImageCard = (propId, furId, a, b) => {
  ...
  document.querySelector(".readytosell__imageline--big").insertAdjacentHTML("afterBegin", readyToSellBigImgString);
  ...
  document.querySelector(".readytosell__description--details").insertAdjacentHTML("afterBegin", readyToSellDetailsString);
...
document.querySelector(".readytosell__imageline--small").insertAdjacentHTML("afterBegin", readyToSellSmallImgString);
}
...
const intervalId = setInterval(() => {
  ...
  if (furId === 2) { furId = 0; } else { furId++; }
  if (furId + 1 <= 2) { a = furId + 1; } else { a = 0; }
  if (a + 1 <= 2) { b = a + 1; } else { b = 0; }
  readyToSellImageCard(propId, furId, a, b);
}, 1500);
document.querySelector(`.readytosell__imageline`).addEventListener("mouseenter", () =>
  setTimeout(() => {
...
document.querySelector(`.readytosell__imageline`).addEventListener("click", (event) => {
...
document.querySelector(".review__imageline").innerHTML = "";
  ...
  `;
  document.querySelector(".review__imageline").insertAdjacentHTML("afterBegin", reviewImgString);
}
reviewImages(reviewLeft, reviewCentral, reviewRight);

document.querySelector(".review__imagelinebullets").addEventListener("click", (event) => {
  if (event.target.dataset.tabname === 'righthouse') {reviewCentral = 8; reviewRight = 6; reviewLeft = 7;}
  if (event.target.dataset.tabname === 'lefthouse') {reviewCentral = 6; reviewRight = 7; reviewLeft = 8;}
  if (event.target.dataset.tabname === 'centerhouse') { reviewCentral = 7; reviewRight = 8; reviewLeft = 6; }
...
let xStart = 0;
let xFinish = 0;
document.querySelector(".review__imageline ").addEventListener("mousedown", (event) => {

  xStart = event.clientX;
   
  }  
) 
document.querySelector(".review__imageline ").addEventListener("touchstart", (event) => {
...
document
 ...
 const determDirection = (difference) => { if (difference > 0 && reviewCentral > 6) {
   reviewCentral--;
  ...
  reviewCentral >= 7
    ...
    : (datatext = "lefthouse");
  document.querySelector( `.review__imagelinebullets--bullet[data-tabname="${datatext}"]`).classList.add("activebullet");
  }


document.querySelector(".subscribe__block--central-emailinput-input").addEventListener("focus", (event) => {

  document.querySelector(".subscribe__block--central-emailinput-input").value = '';

});

document.querySelector(".subscribe__block--central-emailinput-search").addEventListener("click", (event) => {
  
  if (document.querySelector(".subscribe__block--central-emailinput-input").value.includes('@'))
  {
    inputAlarmer(`You have successfully subscribed!</br> e-mail:"${document.querySelector(".subscribe__block--central-emailinput-input").value}"<br/>THANKS!`);
    
  } 
 else if(document.querySelector(".subscribe__block--central-emailinput-input").value !== 'Your email here!')
  {  inputAlarmer(`"${document.querySelector(".subscribe__block--central-emailinput-input").value}" <br/> is NOT VALID E-MAIL. <br/>Try again!`); }
  
  document.querySelector(".subscribe__block--central-emailinput-input").value = 'Your email here!'
});

document.querySelector(".subscribe__block--central-emailinput-input").addEventListener("blur", (event) => {
if(document.querySelector(".subscribe__block--central-emailinput-input").value ==='')
  {document.querySelector(".subscribe__block--central-emailinput-input").value = "Your email here!";}
